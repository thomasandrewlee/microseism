...
use_baro = true
METAR_jld_file = string(user_str,"Downloads/baro_METAR/BOS_baro_19430205_20240625.jld") # METAR baro data from readMETAR.jl 
baro_data_sac = [string(user_str,"Downloads/hrvbaro.550054/30/"),] # directories of sac
baro_save_file = string(user_str,"Desktop/MAI/HRV_BHZ_2010_2022_METAR_BOS_barosave.jld")
     ...
     function paramspacefit(xdat,ydat,angles,Nrbins,linewidth,N_trends,distweight,Nweight)
        ...
        for i = 1:lastindex(xdat)
            for j = 1:lastindex(angles)
                ...
                tmpval = length(gidx)
                tmpval = tmpval - distweight*length(gidx)*(mean(tmpdists[gidx])/xylength[j])
                tmpval = tmpval - Nweight*length(gidx)*(length(gidx)/length(xdat))
                push!(val, tmpval)
                push!(theta, angles[j])
                push!(r, -c)
            end
